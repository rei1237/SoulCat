...
for(const [name,type,options] of [['chromium',chromium,{channel:'chrome'}],['webkit',webkit,{}]]){
  ...
  try{
    for(const width of (process.env.CHECKOUT_BROWSER?[360]:[360,390,430,1280])){
      for(const method of ['CARD','KAKAOPAY']){
        ...
        await context.route('**/*',async route=>{
          ...
          if(url.pathname.endsWith('/payments/verify')){
            ...
            data=status===401?{code:state,message:'로그인 후 확인해 주세요.'}:{status:state,requestId:state==='PAID'?'request':undefined,returnPath:'/fortune/'};
          }
        ...
        await page.goto(origin+'/fortune/?profile=profile');
        ...
        const ticket={orderId:'order',paymentId:'payment',productId:'saju_mackerel',profileId:'profile',returnPath:'/fortune/',generation:'pending',createdAt:Date.now()};
        ...
        await page.goto(origin+'/fortune/?orderId=order&paymentId=payment');
        ...
        const tab=await context.newPage();await tab.goto(origin+'/fortune/?orderId=order&paymentId=payment');
        await expect(tab.locator('.checkout-recovery')).toContainText('복귀 정보를 확인하지 못했어요');expect(verifies).toBe(1);await tab.close();
        await page.evaluate(t=>localStorage.setItem('soulcat:checkout:payment',JSON.stringify(t)),ticket);
        await page.goto(origin+'/fortune/?orderId=wrong&paymentId=payment');
        await expect(page.locator('.checkout-recovery')).toContainText('복귀 정보를 확인하지 못했어요');expect(verifies).toBe(1);
        state='CANCELLED';await page.goto(origin+'/fortune/?orderId=order&paymentId=payment');
        ...
        await page.goto(origin+'/library/?orderId=order&paymentId=payment');
        await expect(page.getByRole('link',{name:'로그인하고 이어가기'})).toHaveAttribute('href',/orderId%3Dorder/);
...
await fs.writeFile('docs/checkout-ui-verification.json',JSON.stringify({generatedAt:new Date().toISOString(),realDevice:false,realPG:false,cases:proof},null,2)+'\n');
