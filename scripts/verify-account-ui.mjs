...
try{for(const width of [360,390,430,1280]){for(const scenario of ['complete','phone','all','outage']){
 ...
 if(width===390&&scenario==='complete'){const tab=await context.newPage();await tab.goto(origin+'/yeongnyangi/library/');await expect(tab.locator('.service-account-bar')).toContainText('영냥테스트님 · 로그인됨');await tab.close();await page.locator('.header-actions .session-menu').getByRole('button',{name:'로그아웃'}).click();await expect(page.locator('.header-actions').getByRole('button',{name:'로그인',exact:true})).toBeVisible();await page.goto(origin+'/yeongnyangi/library/');await page.locator('.service-account-bar').getByRole('button',{name:'로그인',exact:true}).click();await expect(page.locator('.service-account-bar dialog').getByRole('link',{name:'카카오로 로그인'})).toHaveAttribute('href',/next=%2Fyeongnyangi%2Flibrary%2F/);logged=true;}
