...
const origin='http://127.0.0.1:8791/yeongnyangi',proof=[],browser=await chromium.launch({channel:'chrome',headless:true});
try{for(const width of [360,390,430,1280]){
 ...
 await page.goto(origin+'/');await page.getByRole('button',{name:/내 운명 깊게 보기/}).click();await expect(page.locator('.fish-catalog-list a')).toHaveCount(4);await expect(page.locator('.panel-body .primary-cta')).toHaveCount(0);expect((await new AxeBuilder({page}).include('.fish-catalog').withTags(['wcag2a','wcag2aa']).analyze()).violations).toEqual([]);
