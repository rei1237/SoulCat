  ...
  return (
    <main className="fortune-shell" data-library-status={httpStatus}>
      ...
      {orders.filter(o=>['PENDING','PAID'].includes(o.status)).map(o=><p key={o.id}>주문 {o.id.slice(0,8)} · <button disabled={!!checking} onClick={async()=>{
        setChecking(o.id);try{const d=await apiRequest('payments/verify',{orderId:o.id,paymentId:o.payment_id});if(d.status==='PAID'&&d.requestId)window.location.assign('/yeongnyangi/fortune/?request='+encodeURIComponent(d.requestId));else setNotice('결제가 완료되지 않았어요. 잠시 후 다시 확인해 주세요.');}catch(e){setNotice((e as Error).message);}finally{setChecking('');}
