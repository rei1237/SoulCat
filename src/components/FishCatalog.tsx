...
export default function FishCatalog({fusionOnly=false}:{fusionOnly?:boolean}) {
 const [products,setProducts]=useState<Product[]>([]),[failed,setFailed]=useState(false);
 ...
 return <section className="fish-catalog" aria-label={fusionOnly?'초융합 상담 선택':'생선 오마카세 가격표'}><h2>{fusionOnly?'서로 다른 시선으로 읽는 나':'영냥이의 생선 오마카세 가격표'}</h2><p>한 분야를 깊게, 또는 여러 체계의 공통점과 차이까지.</p>{failed&&<p role="alert">가격표를 불러오지 못했어요. 새로고침해 주세요.</p>}<div className="fish-catalog-grid">{items.map(p=><a key={p.id} href={p.readingKind==='single'?`/yeongnyangi/fortune/?domain=${p.domain}&fish=${p.fishId}`:`/yeongnyangi/fortune/?product=${p.id}`}><img src={p.image} width={220} height={180} alt="" loading="lazy"/><span>{fusionOnly?p.name:p.fishName}</span><strong>{p.priceKRW.toLocaleString('ko-KR')}원</strong><small>{p.chapterCount}챕터 · {p.readingKind==='single'?'선택한 한 분야':p.readingKind==='pair'?'두 체계 교차분석':'여섯 체계 전체 통합'}</small><p>{depthDescriptions[p.fishId]}</p><b>차트부터 살펴보기 →</b></a>)}</div></section>;
}
