...
export async function apiRequest(path:string,body?:object) {
  const response=await fetch(`/api/yeongnyangi/${path}`,{credentials:'same-origin',method:body?'POST':'GET',headers:body?{'content-type':'application/json'}:undefined,body:body?JSON.stringify(body):undefined});
...
export async function launchCheckout(order:CheckoutOrder,customer:{fullName:string;phoneNumber:string;email:string},storage=localStorage,requestPayment: (input:PaymentInput)=>ReturnType<typeof import('@portone/browser-sdk/v2').requestPayment> = async input=>(await import('@portone/browser-sdk/v2')).requestPayment(input),verify=apiRequest) {
  const ticket=saveTicket(storage,order);
  const result=await requestPayment({...order.payment,customer});
