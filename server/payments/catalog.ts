...
function product(id:string,systems:DomainId[],fishId:PackageId):Product{return {id,domain:systems[0],fishId,packageId:fishId,fishName:packages[fishId].name,priceKRW:packages[fishId].priceKRW,currency:'KRW',image:`/_soulcat/assets/fish/${fishId}.webp`,reactionAsset:`/_soulcat/assets/fish/reaction-${fishId}.webp`,resultType:'consultation-v3',enabled:true,cdFeatureKey:`yeongnyangi-${id.replace(/_/g,'-')}`,readingKind:systems.length===1?'single':systems.length===2?'pair':'all',systems,manifestVersion:MANIFEST_VERSION,chapterCount:readingChapterCount(systems[0],fishId),name:systems.map(d=>systemNames[d]).join(' + ')};}
