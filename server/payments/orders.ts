...
export async function createOrder(
  ...
  await db
    .prepare(
      "INSERT INTO orders (id,user_id,profile_id,product_id,amount,currency,payment_id,idempotency_key,status,created_at) VALUES (?,?,?,?,?,?,?,?,'PENDING',?) ON CONFLICT DO NOTHING",
    )
    .bind(
      id,
      userId,
  ...
  const order = await db
    .prepare("SELECT * FROM orders WHERE user_id=? AND idempotency_key=? ORDER BY created_at LIMIT 1")
    .bind(userId, key)
    .first<PaymentOrder>();
  if (
    !order ||
...
// D1 batch 는 트랜잭션이라 주문·권리가 반쯤 부여되는 일은 없다.
