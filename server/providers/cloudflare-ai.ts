...
export interface AiBinding {
  run(model: string, input: object): Promise<{ response?: unknown }>;
}
    ...
    try {
      const output = await Promise.race([
        this.ai.run(this.model, {
          messages: messages(r),
          max_tokens: 4096,
          response_format: { type: "json_schema", json_schema: r.outputSchema },
        }),
        new Promise<never>((_, reject) => {
          timer = setTimeout(
            () => reject(new FortuneError("LLM_TIMEOUT", 504)),
            60000,
          );
        }),
      ]);
