...
test('new output cap reaches the transport and old budget attestations are rejected',async()=>{
 ...
 const saved={...productManifest(getProduct('saju_mackerel'))[0],outputTokens:4096};await new StructuredChapterProvider({async generate(r){request=r;return {result:{},provider:'mock',model:'fixture'};}}).generateChapter({chapter:saved,analysis,previous:[]});assert.equal(request.maxOutputTokens,8192,'a book saved before the cap increase retries with the current cap');
 let cap=0,thinking=0;const provider=new GeminiProvider('fixture','fixture',async(_url,init)=>{const config=JSON.parse(String(init?.body)).generationConfig;cap=config.maxOutputTokens;thinking=config.thinkingConfig?.thinkingBudget;return Response.json({candidates:[{finishReason:'STOP',content:{parts:[{text:'{}'}]}}]});},60000,8192);await provider.generate(request);assert.equal(cap,8192);assert.equal(thinking,4096);assert.throws(()=>new GeminiProvider('fixture','fixture').outputTokens(request),/LLM_OUTPUT_LIMIT/);
 for(const [body,code] of [[{candidates:[{finishReason:'MAX_TOKENS'}]},'MAX_TOKENS'],[{promptFeedback:{blockReason:'SAFETY'}},'SAFETY'],[{candidates:[{finishReason:'bad reason'}]},'OTHER']] as const)await assert.rejects(new GeminiProvider('fixture','fixture',async()=>Response.json(body),60000,8192).generate(request),new RegExp(`^Error: INCOMPLETE_PROVIDER_RESPONSE_${code}$`));
